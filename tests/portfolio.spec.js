...
test("portfolio listing enlaza cada tarjeta a su ficha propia", async ({
  page,
}) => {
  ...
  for (const projectSlug of [
    ...
    test(`${projectSlug} carga con SEO y contenido base`, async ({ page }) => {
      ...
      await expect(page.locator('link[rel="canonical"]')).toHaveAttribute(
        "href",
        `https://webfuengirola.com/portfolio/${project.slug}/`,
      );
      ...
      ).toHaveAttribute("content", project.seoDescription);
      ...
      await expect(page.locator(".project-breadcrumb")).toContainText(
        "Casos de éxito",
      );
      await expect(page.locator(".project-detail__image")).toHaveAttribute(
        "alt",
        project.imageAlt,
      );
      await expect(
        page.locator(".project-subhero__actions .btn--primary"),
      ).toHaveAttribute("href", project.url);
      await expect(
        page.locator(".project-subhero__actions .btn--primary"),
      ).toContainText(project.urlLabel);
      const productCategory = productCategories.find(
        (item) => item.slug === project.productCategorySlug,
      );
      if (productCategory) {
        await expect(page.locator(".project-detail__facts")).toContainText(
...
test("sitemap incluye casos, categorías de producto y excluye páginas no indexables", async ({
  ...
  expect(xml).not.toContain(
    "https://webfuengirola.com/servicios/automatizacion/",
  );
  expect(xml).not.toContain(
    "https://webfuengirola.com/servicios/aplicaciones-web/",
  );
  expect(xml).not.toContain("https://webfuengirola.com/recursos/herramientas/");
  expect(xml).not.toContain("https://webfuengirola.com/recursos/guias/");
  expect(xml).not.toContain("https://webfuengirola.com/recursos/checklists/");
});
...
test("ficha de proyecto mantiene CTA visible en móvil", async ({ page }) => {
  ...
  await expect(
    page.getByRole("link", { name: /quiero algo as[ií]/i }),
  ).toBeVisible();
  await expect(page.locator(".project-detail__cta")).toBeVisible();
});

test("la imagen principal de FisioApp enlaza a la demo comercial", async ({
  page,
}) => {
  await page.goto("/portfolio/fisioapp-panel-clinica/");

  await expect(page.locator(".project-detail__image-card")).toHaveAttribute(
    "href",
    "https://webfuengirola.com/portfolio/demo.html",
  );
  await expect(page.locator(".project-detail__image-card")).toHaveAttribute(
    "aria-label",
    /probar demo/i,

...
export function getMonthlyStatReportConfig(env = process.env) {
  const baseUrl = env.STAT_REPORT_UMAMI_URL;
  const password = env.STAT_REPORT_UMAMI_PASSWORD;
  const reportTo = env.STAT_REPORT_EMAIL_TO || env.RESEND_TO_EMAIL;

  if (!baseUrl || !password) {
    throw new Error("STAT_REPORT_UMAMI_URL and STAT_REPORT_UMAMI_PASSWORD are required");
  }
  if (!reportTo) {
  ...
  return {
    baseUrl,
    username: env.STAT_REPORT_UMAMI_USERNAME || "admin",
    password,
    reportTo,
  };
}
...
export async function deliverMonthlyStatReport({
  ...
  const claimed = await claimDelivery({ monthKey, emailTo, claimToken });
...
export async function resolveReportSites({ baseUrl, token, sites, fetchImpl = fetch }) {
  ...
  let response;
  try {
    response = await requestJson(apiUrl(baseUrl, "/api/websites", { pageSize: 200 }), {
      token,
      fetchImpl,
    });
  } catch {
...
export async function processMonthlyStatReport({
  ...
  const storageRef = await saveReport({

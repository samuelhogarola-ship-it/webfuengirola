...
export async function resolveReportSites({
  baseUrl,
  token,
  sites,
  fetchImpl = fetch,
  ...
  const response = await requestJson(
    apiUrl(baseUrl, "/api/websites", { pageSize: 200 }),
    {
      token,
      fetchImpl,
    },
  ...
  return sites.map((site) => {
    ...
    const match = websites.find((website) => {
      ...
      return (
        id &&
        (domain === site.domain ||
          name.toLowerCase() === site.label.toLowerCase())
      );
    });
    return match
      ? { ...site, websiteId: match.id || match.websiteId || match.website_id }
      : site;
  });
}
...
export async function fetchUmamiSiteSummary({
  ...
  try {
    ...
    const [stats, topPages, topReferrers, topCountries, devices] =
      await Promise.all([
        requestJson(
          apiUrl(baseUrl, `/api/websites/${site.websiteId}/stats`, baseParams),
          {
            token,
            fetchImpl,
          },
        ...
        requestJson(
          apiUrl(baseUrl, `/api/websites/${site.websiteId}/metrics`, {
            ...baseParams,
            type: "url",
            limit: 8,
          }),
...
export function renderMonthlyStatReport({
  range,
  siteReports,
  ...
  for (const report of siteReports) {
    ...
    lines.push(
      metricLine(
        "Páginas vistas",
        pageviews,
        previousValue("pageviews", stats),
      ),
    );
    lines.push(
      metricLine("Visitantes", visitors, previousValue("visitors", stats)),
    );
...
export async function processMonthlyStatReport({
  ...
  const markdown = renderMonthlyStatReport({
    range,
    siteReports,
    generatedAt: now,
  });

...
const casosDir = path.join(rootDir, "casos");
      ...
      <nav class="nav" id="nav" aria-label="Navegación principal">
        <ul class="nav__list">
          ${navItems
            .map(
              (item) =>
                `<li><a href="${prefix}${item.href}" class="nav__link${item.key === activePage ? " nav__link--active" : ""}">${item.label}</a></li>`,
            )
            .join("\n          ")}
        </ul>
      </nav>
    ...
    <div class="container footer__inner">
      ...
      <div class="footer__col">
        ...
        <ul class="footer__links">
          <li><a href="${prefix}servicios/diseno-web/">Diseño Web</a></li>
          <li><a href="${prefix}servicios/seo-local/">SEO Local</a></li>
          <li><a href="${prefix}servicios/mantenimiento/">Mantenimiento</a></li>
          <li><a href="${prefix}servicios/automatizacion/">Automatización e IA</a></li>
          <li><a href="${prefix}servicios/aplicaciones-web/">Apps a medida</a></li>
        </ul>
      </div>
      <div class="footer__col">
      ...
      <div class="footer__col">
        ...
        <ul class="footer__links">
          <li><a href="${prefix}legal.html#aviso-legal">Aviso legal</a></li>
          <li><a href="${prefix}legal.html#privacidad">Política de privacidad</a></li>
          <li><a href="${prefix}legal.html#cookies">Política de cookies</a></li>
        </ul>
      </div>
      <div class="footer__col">
        <h4 class="footer__col-title">Contacto</h4>
        <ul class="footer__links">
          <li><a href="${prefix}contacto/">Página de contacto</a></li>
      ...
      (c) => `
        <article class="portfolio-card">
          <div class="portfolio-card__img-wrap">
            <img src="${prefix}${c.image}" alt="${escapeHtml(c.ogAlt)}" class="portfolio-card__img" loading="lazy" width="400" height="250" />
          </div>
          <div class="portfolio-card__body">
            <div style="display:flex;gap:0.5rem;margin-bottom:0.75rem;flex-wrap:wrap">
              <span class="tag tag--blue">${escapeHtml(c.sector)}</span>
              <span class="tag tag--purple">${escapeHtml(c.serviceLabel)}</span>
            </div>
            <h3 class="portfolio-card__title">${escapeHtml(c.title)}</h3>
            <p class="portfolio-card__desc">${escapeHtml(c.headline)}</p>
            <p style="font-size:0.85rem;color:var(--color-text-muted);margin-top:0.5rem">${escapeHtml(c.result)}</p>
          </div>
          <div class="portfolio-card__footer">
            <a href="${prefix}casos/${c.slug}/" class="btn btn--outline">Ver caso →</a>
          </div>
        </article>`,
    )
    .join("\n");
...
${renderFooter(prefix)}
  ...
  ensureDir(casosDir);
  fs.writeFileSync(path.join(casosDir, "index.html"), html, "utf8");
  console.log("✓ casos/index.html");
}

function buildCasePage(caso) {
  const prefix = "../../";
  ...
  const relatedCasesHtml =
    ...
    <section class="section" style="background: var(--color-surface-warm)">
      <div class="container">
        <div class="section-header">
          <span class="section-label">Del mismo sector</span>
        ...
        <div class="portfolio__grid" style="margin-top: 2rem">
          ...
          <article class="portfolio-card">
            <div class="portfolio-card__body">
              <span class="tag tag--blue" style="margin-bottom:0.75rem;display:inline-block">${escapeHtml(rc.sector)}</span>
              <h3 class="portfolio-card__title">${escapeHtml(rc.title)}</h3>
              <p class="portfolio-card__desc">${escapeHtml(rc.headline)}</p>
            </div>
            <div class="portfolio-card__footer">
              <a href="${prefix}casos/${rc.slug}/" class="btn btn--outline">Ver caso →</a>
            </div>
          </article>`,
            )
            .join("")}
...
${renderHead({
  ...
  canonical,
  ogTitle: caso.metaTitle,
  ogDescription: caso.metaDescription,
...
${renderHeader(prefix, "casos")}
  ...
  <main>
    <section class="section" style="padding-top: calc(var(--header-h) + 4rem)">
      <div class="container">
        ...
        <div style="display:flex;gap:0.5rem;margin-bottom:1.5rem;flex-wrap:wrap">
          <span class="tag tag--blue">${escapeHtml(caso.sector)}</span>
          <span class="tag tag--purple">${escapeHtml(caso.serviceLabel)}</span>
        </div>

        <div style="max-width: 720px">
          <h1 class="section-title">${escapeHtml(caso.headline)}</h1>
          <p class="section-desc" style="margin-top: 1rem">${escapeHtml(caso.summary)}</p>
        </div>

        <div class="project-detail__panel" style="margin-top: 3rem; padding: 2rem; border-radius: var(--radius-md); background: var(--color-surface-warm)">
          <p style="font-size:0.85rem;text-transform:uppercase;letter-spacing:0.1em;color:var(--color-text-muted);margin-bottom:0.5rem">Resultado</p>
          <p style="font-size: 1.1rem; font-weight: 600">${escapeHtml(caso.result)}</p>
        </div>
    ...
    <section class="section" style="background: var(--color-surface-warm)">
      <div class="container">
        <div class="section-header">
          <span class="section-label">Servicio utilizado</span>
          <h2 class="section-title" style="font-size: clamp(1.25rem, 2.5vw, 1.75rem)">${escapeHtml(caso.serviceLabel)}</h2>
        </div>
        <div style="margin-top: 1.5rem; display:flex; gap: 1rem; flex-wrap:wrap">
          <a href="${prefix}servicios/${caso.service}/" class="btn btn--outline">Ver servicio →</a>
          <a href="https://wa.me/34622923988?text=${encodeURIComponent(caso.whatsappText)}" class="btn btn--primary" target="_blank" rel="noopener noreferrer">¿Tu negocio tiene un reto similar? →</a>
        </div>
      </div>
    </section>
...
${renderFooter(prefix, caso.whatsappText)}
</body>
</html>`;

  ensureDir(dir);
  fs.writeFileSync(path.join(dir, "index.html"), html, "utf8");
  console.log(`✓ casos/${caso.slug}/index.html`);
}
...
console.log(`\n✅ ${cases.length + 1} páginas de casos generadas.`);
